...
public class Main {
    public static void main(String[] args) {
        ...
        int i,j;
        System.out.println("Studiourile care au mai multe de doua filme sunt: ");
        for (i = 0; i < studioDatabase.length; i++) {
            if (studioDatabase[i].film.length > 2) System.out.println(studioDatabase[i].nume + " ");
        }
        System.out.println("Studioul care are actorul cu 2 oscaruri este: ");
        for (i = 0; i < studioDatabase.length; i++) {
            studioDatabase[i].printActorNames();
        }

        System.out.println("Filmele in care joaca actori 50+ sunt: ");
        for (i = 0; i < studioDatabase.length; i++) {
            for( j = 0; j<studioDatabase[i].film.length; j++)
                 studioDatabase[i].film[j].actorsAge();
        }
    }
}
